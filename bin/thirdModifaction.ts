...
const actionTypes = [
  'LISTUPDATEPET', 'ADDUPDATEPET', 'UPDATEUPDATEPET', 'DELETEUPDATEPET',
  'LISTADDPET', 'ADDADDPET', 'UPDATEADDPET', 'DELETEADDPET',
  'LISTFINDPETSBYSTATUS', 'ADDFINDPETSBYSTATUS', 'UPDATEFINDPETSBYSTATUS', 'DELETEFINDPETSBYSTATUS',
  'LISTDELETEPET', 'ADDDELETEPET', 'UPDATEDELETEPET', 'DELETEDELETEPET'
];

actionTypes.forEach(type => {
  actionHandlers[type] = handleLoading;
});

actionHandlers['RESET_PET_STATE'] = () => initialState;
...
const petReducer = (state = initialState, action: petAction): petState => {
  ...
  if (type.endsWith(SUCCESS_SUFFIX)) {
    switch (true) {
      case type.startsWith('ADD'):
        return handleAddSuccess(state, action);
      case type.startsWith('UPDATE'):
        return handleUpdateSuccess(state, action);
      case type.startsWith('DELETE'):
